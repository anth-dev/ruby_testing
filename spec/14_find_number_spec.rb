...
describe FindNumber do
  ...
  describe '#make_guess' do
    ...
    context 'when min is 5 and max is 9' do
      subject(:game_min_five_max_nine) { described_class.new(5, 9, number_guessing) }

      it 'returns 7' do
        guess = game_min_five_max_nine.make_guess
        expect(guess).to eq(7)
      end
    end

    context 'when min is 8 and max is 9' do
      subject(:game_min_eight_max_nine) { described_class.new(8, 9, number_guessing) }

      it 'returns 8' do
        guess = game_min_eight_max_nine.make_guess
        expect(guess).to eq(8)
      end
    end

    context 'when min is 0 and max is 3' do
      subject(:game_min_zero_max_three) { described_class.new(0, 3, number_guessing) }

      it 'returns 1' do
        guess = game_min_zero_max_three.make_guess
        expect(guess).to eq(1)
      end
    end

    context 'when min and max both equal 3' do
      subject(:game_min_three_max_three) { described_class.new(3, 3, number_guessing) }

      it 'returns 3' do
        guess = game_min_three_max_three.make_guess
  ...
  describe '#game_over?' do
    let(:answer_of_three) { double('random_number', value: 3) }
    ...
    context 'when guess and random_number are equal' do
      ...
      subject(:game_is_over) { described_class.new(0, 9, answer_of_three, 3) }
    ...
    context 'when guess and random_number are not equal' do
      subject(:game_is_not_over) { described_class.new(0, 9, answer_of_three, 4) }
      it 'is not game over' do
        expect(game_is_not_over).not_to be_game_over
      end
    end
  end
  ...
  describe '#update_range' do
    ...
    context 'when the guess is 7, min=5, and max=8' do
      ...
      before do
        first_edge_case.update_range
      end

      it 'updates min to the same value as max' do
        expect(first_edge_case.min).to eq(8)
      end

      it 'does not update max' do
        expect(first_edge_case.max).to eq(8)
      end
    end
  end
end
