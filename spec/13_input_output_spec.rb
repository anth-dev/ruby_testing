...
describe NumberGame do
  ...
  describe '#game_over?' do
    ...
    context 'when user guess is not correct' do
      ...
      subject(:game_continues) { described_class.new(7, '4') }
  ...
  describe '#player_turn' do
    ...
    context 'when user inputs two incorrect values, then a valid input' do
      before do
        ...
        letter_t = 't'
        valid_input = '3'
        allow(game_loop).to receive(:player_input).and_return(letter_a, letter_t, valid_input)
      end
  ...
  describe '#final_message' do
    ...
    context 'when count is 2-3' do
      subject(:game_won_in_two_to_three) { described_class.new(9, '9', 3) }
      # remove the 'x' before running this test
      it 'outputs correct phrase' do
        congrats_phrase = "Congratulations! You picked the random number in 3 guesses!\n"
        expect { game_won_in_two_to_three.final_message }.to output(congrats_phrase).to_stdout
      end
    end
    ...
    context 'when count is 4 and over' do
      subject(:game_won_in_five) { described_class.new(8, '8', 5)}
      # remove the 'x' before running this test
      it 'outputs correct phrase' do
        hard_phrase = "That was hard. It took you 5 guesses!\n"
        expect { game_won_in_five.final_message }.to output(hard_phrase).to_stdout
      end
    end
  end
end
